...
func SaveSegmentSnapshot(w io.Writer, indexName string, index Index, filterName string, filter Filter) error {
	if w == nil {
		return fmt.Errorf("fts: save snapshot: nil writer")
	}
	if index == nil {
		return fmt.Errorf("fts: save snapshot: nil index")
	}
	if indexName == "" {
		return fmt.Errorf("fts: save snapshot: empty index name")
	}

	indexCodec, ok := indexCodecByName(indexName)
	if !ok {
		return fmt.Errorf("fts: save snapshot: unknown index codec %q", indexName)
	}

	var indexPayload bytes.Buffer
	if err := indexCodec.save(index, &indexPayload); err != nil {
		return fmt.Errorf("fts: save snapshot: encode index %q: %w", indexName, err)
	}
	...
	if filter != nil {
		...
		if err := filterCodec.save(filter, &filterPayload); err != nil {
			return fmt.Errorf("fts: save snapshot: encode filter %q: %w", filterName, err)
		}
	...
	if err := gob.NewEncoder(w).Encode(envelope); err != nil {
		return fmt.Errorf("fts: save snapshot: encode envelope: %w", err)
	}

	return nil
}

func LoadSegmentSnapshot(r io.Reader) (*LoadedSnapshot, error) {
	if r == nil {
		return nil, fmt.Errorf("fts: load snapshot: nil reader")
	}

	var envelope snapshotEnvelope
	if err := gob.NewDecoder(r).Decode(&envelope); err != nil {
		return nil, fmt.Errorf("fts: load snapshot: decode envelope: %w", err)
	}

	if envelope.Version != snapshotVersion {
		return nil, fmt.Errorf("fts: load snapshot: unsupported version %d", envelope.Version)
	}
	if envelope.IndexName == "" {
		return nil, fmt.Errorf("fts: load snapshot: empty index name")
	}

	indexCodec, ok := indexCodecByName(envelope.IndexName)
	if !ok {
		return nil, fmt.Errorf("fts: load snapshot: unknown index codec %q", envelope.IndexName)
	}

	index, err := indexCodec.load(bytes.NewReader(envelope.IndexPayload))
	if err != nil {
		return nil, fmt.Errorf("fts: load snapshot: decode index %q: %w", envelope.IndexName, err)
	}
	...
	if envelope.FilterName != "" {
		...
		if !ok {
			return nil, fmt.Errorf("fts: load snapshot: unknown filter codec %q", envelope.FilterName)
		}

		filter, err := filterCodec.load(bytes.NewReader(envelope.FilterPayload))
		if err != nil {
			return nil, fmt.Errorf("fts: load snapshot: decode filter %q: %w", envelope.FilterName, err)
		}
...
func NewFromSnapshot(r io.Reader, keyGen KeyGenerator, opts ...Option) (*Service, error) {
	loaded, err := LoadSegmentSnapshot(r)
	if err != nil {
		return nil, err
	}

	builtOpts := make([]Option, 0, len(opts)+1)
	if loaded.Filter != nil {
		builtOpts = append(builtOpts, WithFilter(loaded.Filter))
	}
	builtOpts = append(builtOpts, opts...)

	return New(loaded.Index, keyGen, builtOpts...), nil
}
